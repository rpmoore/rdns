...
| Local DNS entries can accidentally bypass policy or hide stale cache answers. | Evaluate deny and malicious-domain policy before local entries, generate exact-name `A`/`AAAA` answers from structured entry data, require `.local` warnings and public-address acknowledgement, and invalidate affected cache entries or advance generation on changes. | `02-resolver-cache.md`, `03-policy-blocking.md`, `05-persistence-config.md`, `06-admin-api-ui.md` | Milestone 6 |
...
- Local DNS entry precedence and cache invalidation are blockers before user-managed local host overrides.
