...
2. Add checked packet reading and structured parse errors.
   - Implements: [Existing Code To Rework](plan/01-dns-protocol.md#existing-code-to-rework), [Error Model](plan/01-dns-protocol.md#domain-model), [Primary Risks](plan/00-overview.md#primary-risks-in-the-existing-code).
...
11. Add static runtime configuration for DNS listen and upstreams.
    - Implements: [Upstream Resolution](plan/02-resolver-cache.md#upstream-resolution), [Configuration Reload](plan/05-persistence-config.md#configuration-reload), [Deployment Notes](plan/07-operations-testing.md#deployment-notes).
...
13. Implement upstream UDP forwarding.
    - Implements: [Upstream Resolution](plan/02-resolver-cache.md#upstream-resolution), [Failure Behavior](plan/02-resolver-cache.md#failure-behavior), [Security Considerations](plan/07-operations-testing.md#security-considerations).
...
14. Add deterministic upstream failover and deadlines.
    - Implements: [Initial Upstream Strategy](plan/02-resolver-cache.md#upstream-resolution), [Failure Behavior](plan/02-resolver-cache.md#failure-behavior), [Upstream behavior concern](plan/09-reviewer-concerns.md#concern-response-matrix).
...
## Phase 4: Local Policy Blocking

24. Add domain normalization and selector models.
    - Implements: [Domain Normalization](plan/03-policy-blocking.md#domain-normalization), [Domain normalization concern](plan/09-reviewer-concerns.md#concern-response-matrix), [Milestone 4](plan/08-implementation-roadmap.md#milestone-4-local-policy-blocking).
    - Canonicalize lowercase ASCII/Punycode names without trailing dots and model exact versus subtree selectors explicitly.

25. Add client identity and selector models.
...
26. Implement local deny-rule evaluation.
...
27. Add explicit block response configuration.
...
28. Integrate policy into `ResolveQuery`.
...
29. Add response-aware malicious-domain checks.
...
30. Add policy tests.
    - Implements: [Policy Tests](plan/03-policy-blocking.md#tests), [Milestone 4 Exit Criteria](plan/08-implementation-roadmap.md#milestone-4-local-policy-blocking).
...
## Phase 5: SQLite Persistence And Runtime Config

31. Choose and isolate the SQLite async strategy.
...
32. Add migrations for durable tables and indices.
...
33. Implement settings, upstream, rule, and query-event repositories.
    ...
    - Shape repository methods around use cases such as loading resolver settings, replacing upstreams, loading policy snapshots, and appending query events.

34. Add immutable runtime snapshots and atomic reload.
...
35. Enforce configuration validation before persistence and publication.
...
36. Add startup migration and degraded-startup behavior.
...
37. Add bounded asynchronous query-event writing.
    ...
    - Send events through a bounded queue or writer task, define overflow behavior, and emit drop/sampling metrics without blocking DNS responses.

38. Add retention maintenance.
    ...
    - Enforce query-event retention, maximum rows, maximum blocklist generations retained per source, and database optimize/vacuum strategy after large churn.

39. Add persistence and reload tests.
    - Implements: [Persistence Tests](plan/05-persistence-config.md#tests), [Milestone 5 Exit Criteria](plan/08-implementation-roadmap.md#milestone-5-sqlite-persistence-and-runtime-config).
...
## Phase 6: External Blocklist Ingestion

40. Add blocklist source and generation persistence.
    - Implements: [Blocklist Inputs](plan/04-blocklist-ingestion.md#inputs), [Proposed Tables](plan/05-persistence-config.md#proposed-tables), [Milestone 6](plan/08-implementation-roadmap.md#milestone-6-external-blocklist-ingestion).
    - Persist source settings, update status, inactive generations, domains, and active generation pointers.

41. Implement safe fetcher adapter.
...
43. Normalize, dedupe, and validate staged domains.
...
45. Implement transactional activation and snapshot swap.
...
46. Add manual refresh service and API-facing status model.
    - Implements: [Update Flow](plan/04-blocklist-ingestion.md#update-flow), [API Scope](plan/06-admin-api-ui.md#api-scope), [Milestone 6 Exit Criteria](plan/08-implementation-roadmap.md#milestone-6-external-blocklist-ingestion).
...
48. Add blocklist ingestion tests.
...
49. Add admin HTTP server and route structure.
    - Implements: [Admin Responsibility](plan/06-admin-api-ui.md#responsibility), [Delivery Implementation](plan/06-admin-api-ui.md#delivery-implementation), [Milestone 7](plan/08-implementation-roadmap.md#milestone-7-admin-api).
...
50. Implement first-run setup and authentication.
...
51. Add CSRF protection and safe bind defaults.
...
52. Implement settings and upstream endpoints.
    - Implements: [API Scope](plan/06-admin-api-ui.md#api-scope), [Configuration Reload](plan/05-persistence-config.md#configuration-reload), [Milestone 7 Tasks](plan/08-implementation-roadmap.md#milestone-7-admin-api).
    - Add typed request/response structs and validation that prevents deleting or disabling the last usable upstream unless degraded mode is explicitly enabled.

53. Implement rule endpoints.
...
54. Implement blocklist source and refresh endpoints.
    - Implements: [API Scope](plan/06-admin-api-ui.md#api-scope), [Blocklist Scheduling](plan/04-blocklist-ingestion.md#scheduling), [Manual refresh roadmap task](plan/08-implementation-roadmap.md#milestone-6-external-blocklist-ingestion).
    - Manage sources, validate URLs with fetcher safety rules, trigger refresh asynchronously, and return update status.

55. Implement status, query-event, and metrics endpoints.
    ...
    - Expose resolver health, listener state, upstream health, cache summary, blocklist freshness, query history, and metrics summary.

56. Add audit-friendly admin change logging.
...
57. Add API security and validation tests.
    - Implements: [Admin API Tests](plan/06-admin-api-ui.md#tests), [Milestone 7 Exit Criteria](plan/08-implementation-roadmap.md#milestone-7-admin-api).
    - Cover first-run setup, unauthenticated mutation rejection, CSRF, session expiry, invariant validation, settings reload, and refresh endpoint behavior.

## Phase 8: Admin UI

58. Add static UI shell served by the admin server.
    - Implements: [Delivery Implementation](plan/06-admin-api-ui.md#delivery-implementation), [Milestone 8](plan/08-implementation-roadmap.md#milestone-8-admin-ui).
...
59. Build the status screen.
    ...
    - Show resolver health, listening addresses, upstream health, cache hit rate, blocklist freshness, and recent query decisions.

60. Build upstream management.
    - Implements: [UI Screens](plan/06-admin-api-ui.md#ui-screens), [Upstream Resolution](plan/02-resolver-cache.md#upstream-resolution).
    - Add, edit, enable, disable, reorder, validate, and test upstream resolvers.

61. Build rules management.
...
63. Build query history.
    ...
    - Filter recent events by client, domain, decision, cache status, and time; show reason codes and sampling/drop indicators.

64. Build settings management.
...
65. Add UI smoke and validation tests.
    - Implements: [Admin API/UI Tests](plan/06-admin-api-ui.md#tests), [Milestone 8 Exit Criteria](plan/08-implementation-roadmap.md#milestone-8-admin-ui).
    - Smoke-test primary screens, core forms, validation errors, unauthenticated redirects, and API-confirmed active-state updates.

## Phase 9: TCP Listener And Operational Hardening

66. Add DNS TCP listener delivery adapter.
    - Implements: [TCP Framing](plan/01-dns-protocol.md#tcp-framing), [Milestone 9](plan/08-implementation-roadmap.md#milestone-9-tcp-and-operational-hardening).
    - Enforce length-prefix framing, maximum message size, read/write timeouts, idle close, and configured connection limits.

67. Share resolver flow between UDP and TCP listeners.
...
68. Add structured logging and metrics coverage.
...
69. Add health-check completeness.
...
70. Add fuzzing targets.
    - Implements: [Fuzzing](plan/01-dns-protocol.md#tests), [Fuzz And Robustness](plan/07-operations-testing.md#test-strategy), [Milestone 9 Exit Criteria](plan/08-implementation-roadmap.md#milestone-9-tcp-and-operational-hardening).
...
72. Add deployment and privilege documentation.
    - Implements: [Deployment Notes](plan/07-operations-testing.md#deployment-notes), [Port 53 concern](plan/09-reviewer-concerns.md#second-review-additions), [Milestone 9 Tasks](plan/08-implementation-roadmap.md#milestone-9-tcp-and-operational-hardening).
...
73. Review defaults and final local-network safety posture.
